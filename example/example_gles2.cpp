...
int main()
{
	...
	window = nvgwin_create(1000, 600, "NanoVG", NVGWIN_PROFILE_GLES2, 0, 0);
	if (!window) {
		nvgwin_shutdown();
		return -1;
	}
	...
	vgOwner = nvg::createGLES2(static_cast<int>(nvg::CreateFlags::Antialias | nvg::CreateFlags::StencilStrokes | nvg::CreateFlags::Debug));
	if (!vgOwner) {
