...
void drawEditBoxNum(nvg::Context& vg, const std::string& text, const std::string& units, float x, float y, float w, float h)
{
    float uw;
...
void drawGraph(nvg::Context& vg, float x, float y, float w, float h, float t)
{
    ...
    vg.beginPath();
    for (i = 0; i < 6; i++) vg.circle(sx[i], sy[i], 4.0f);
    vg.fillColor(nvg::rgba(0, 160, 192, 255));
    vg.fill();
    vg.beginPath();
    for (i = 0; i < 6; i++) vg.circle(sx[i], sy[i], 2.0f);
...
void freeDemoData(nvg::Context& vg, DemoData& data)
{
    ...
    for (i = 0; i < 12; i++) vg.deleteImage(data.images[i]);
}
...
void drawParagraph(nvg::Context& vg, float x, float y, float width, float height, float mx, float my)
{
    ...
    static const std::string kHoverTooltip = "Hover your mouse over the text to see calculated caret position.";
        ...
        for (i = 0; i < nrows; i++) {
            ...
            if (hit) {
                ...
                for (j = 0; j < nglyphs; j++) {
                    ...
                    if (mx >= px && mx < splitX) caretx = glyphs[(size_t)j].x;
                    px = splitX;
                }
                vg.beginPath();
    ...
    if (gutter) {
        ...
        vg.beginPath();
        vg.fillColor(nvg::rgba(255, 192, 0, 255));
        vg.roundedRect((float)((int)bounds[0] - 4), (float)((int)bounds[1] - 2), (float)((int)(bounds[2] - bounds[0]) + 8), (float)((int)(bounds[3] - bounds[1]) + 4),
...
void renderDemo(nvg::Context& vg, float mx, float my, float width, float height, float t, int blowup, DemoData& data)
{
    ...
    switch ((int)(t / 5.0f) % 3) {
        case 0:
            ...
            break;
        case 1:
            vg.lineStyle(static_cast<int>(nvg::LineStyle::Dotted));
            break;
        case 2:
            vg.lineStyle(static_cast<int>(nvg::LineStyle::Glow));
            break;
        default:
            vg.lineStyle(static_cast<int>(nvg::LineStyle::Solid));
    }
