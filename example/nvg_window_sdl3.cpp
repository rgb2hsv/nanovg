...
static int map_sdl_key_to_nvg(SDL_Keycode k)
{
    switch (k) {
        case SDLK_ESCAPE:
            return NVG_KEY_ESCAPE;
        case SDLK_SPACE:
            return NVG_KEY_SPACE;
        case SDLK_S:
            return NVG_KEY_S;
        case SDLK_P:
            return NVG_KEY_P;
...
{
    if (g_error_fn) g_error_fn(0, SDL_GetError());
    (void)what;
}
    ...
    switch (profile) {
        ...
        case NVGWIN_PROFILE_GL3:
            ...
            SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_CORE);
            {
                ...
                if (debug_context) glflags |= SDL_GL_CONTEXT_DEBUG_FLAG;
                SDL_GL_SetAttribute(SDL_GL_CONTEXT_FLAGS, glflags);
            }
#else
            if (debug_context) SDL_GL_SetAttribute(SDL_GL_CONTEXT_FLAGS, SDL_GL_CONTEXT_DEBUG_FLAG);
#endif
            if (msaa_samples > 0) {
                SDL_GL_SetAttribute(SDL_GL_MULTISAMPLEBUFFERS, 1);
                SDL_GL_SetAttribute(SDL_GL_MULTISAMPLESAMPLES, msaa_samples);
...
{
    if (!win) return;
    if (g_active_window == win) g_active_window = nullptr;
            ...
            case SDL_EVENT_KEY_UP:
                if (nw && nw->key_cb) {
                    ...
                    if (key >= 0) {
                        ...
                        if (e.type == SDL_EVENT_KEY_DOWN && e.key.repeat) action = 2;
