...
- **Build** — **CMake** is the supported route; **GLEW** is required via `find_package` or the `external/glew` submodule. **GLFW** is required for **legacy** examples (`example_legacy_*`) and for C++ examples when SDL3 is not used (`external/glfw` submodule or system package). **Premake** (`premake4.lua`) is still present but secondary. Example targets: `example_gl2`, `example_gl3`, `example_fbo` (and GLES variants on non-Windows), plus **`example_legacy_*`** for the original C sources.
