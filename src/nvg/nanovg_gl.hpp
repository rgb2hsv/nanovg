...
inline constexpr int operator&(int a, CreateFlags b) noexcept
{
...
inline constexpr int operator|(int a, ImageFlagsGl b) noexcept
{
    return a | to_underlying(b);
}
...
enum GlUniformLoc { GlUniformLocViewSize, GlUniformLocTex, GlUniformLocFrag, GlUniformLocCount };

enum GlShaderType { NSVG_SHADER_FILLGRAD, NSVG_SHADER_FILLIMG, NSVG_SHADER_SIMPLE, NSVG_SHADER_IMG };
...
static int glnvg__deleteTexture(GLContext* gl, int id)
{
    ...
    for (i = 0; i < gl->ntextures; i++) {
        if (gl->textures[i].id == id) {
            if (gl->textures[i].tex != 0 && (gl->textures[i].flags & ImageFlagsGl::NoDelete) == 0) glDeleteTextures(1, &gl->textures[i].tex);
...
if (lineStyle > 1 && strokeAlpha < strokeThr) discard;
    ...
    if (gl->flags & CreateFlags::Antialias) {
        ...
        if (glnvg__createShader(&gl->shader, "shader", shaderHeader, NULL, fillVertShader, fillFragShader) == 0) return 0;
    }
...
static int glnvg__convertPaint(GLContext* gl, GlFragUniforms* frag, Paint* paint, Scissor* scissor, float width, float fringe, float strokeThr, int lineStyle)
{
    GlTexture* tex = NULL;
...
{
    if (factor == static_cast<int>(BlendFactor::Zero)) return GL_ZERO;
    if (factor == static_cast<int>(BlendFactor::One)) return GL_ONE;
    if (factor == static_cast<int>(BlendFactor::SrcColor)) return GL_SRC_COLOR;
    if (factor == static_cast<int>(BlendFactor::OneMinusSrcColor)) return GL_ONE_MINUS_SRC_COLOR;
    if (factor == static_cast<int>(BlendFactor::DstColor)) return GL_DST_COLOR;
    if (factor == static_cast<int>(BlendFactor::OneMinusDstColor)) return GL_ONE_MINUS_DST_COLOR;
    if (factor == static_cast<int>(BlendFactor::SrcAlpha)) return GL_SRC_ALPHA;
    if (factor == static_cast<int>(BlendFactor::OneMinusSrcAlpha)) return GL_ONE_MINUS_SRC_ALPHA;
    if (factor == static_cast<int>(BlendFactor::DstAlpha)) return GL_DST_ALPHA;
    if (factor == static_cast<int>(BlendFactor::OneMinusDstAlpha)) return GL_ONE_MINUS_DST_ALPHA;
    if (factor == static_cast<int>(BlendFactor::SrcAlphaSaturate)) return GL_SRC_ALPHA_SATURATE;
...
static void glnvg__renderFill(void* uptr, Paint* paint, CompositeOperationState compositeOperation, Scissor* scissor, float fringe, const float* bounds, const Path* paths, int npaths)
{
...
static void glnvg__renderStroke(void* uptr, Paint* paint, CompositeOperationState compositeOperation, Scissor* scissor, float fringe, float strokeWidth, int lineStyle, const Path* paths, int npaths)
{
    GLContext* gl = (GLContext*)uptr;
    GlCall* call = glnvg__allocCall(gl);
    int i, maxverts, offset;
...
static void glnvg__renderTriangles(void* uptr, Paint* paint, CompositeOperationState compositeOperation, Scissor* scissor, const Vertex* verts, int nverts, float fringe)
{
    GLContext* gl = (GLContext*)uptr;
    GlCall* call = glnvg__allocCall(gl);
    GlFragUniforms* frag;
