...
enum class GLNVGshaderType : int {
	FILLGRAD,
	FILLIMG,
	SIMPLE,
	IMG
...
{
	shader->loc[static_cast<int>(GLNVGuniformLoc::VIEWSIZE)] = glGetUniformLocation(shader->prog, "viewSize");
	shader->loc[static_cast<int>(GLNVGuniformLoc::SCISSORMAT)] = glGetUniformLocation(shader->prog, "scissorMat");
	shader->loc[static_cast<int>(GLNVGuniformLoc::SCISSOREXT)] = glGetUniformLocation(shader->prog, "scissorExt");
	shader->loc[static_cast<int>(GLNVGuniformLoc::SCISSORSCALE)] = glGetUniformLocation(shader->prog, "scissorScale");
	shader->loc[static_cast<int>(GLNVGuniformLoc::PAINTMAT)] = glGetUniformLocation(shader->prog, "paintMat");
	shader->loc[static_cast<int>(GLNVGuniformLoc::EXTENT)] = glGetUniformLocation(shader->prog, "extent");
	shader->loc[static_cast<int>(GLNVGuniformLoc::RADIUS)] = glGetUniformLocation(shader->prog, "radius");
	shader->loc[static_cast<int>(GLNVGuniformLoc::FEATHER)] = glGetUniformLocation(shader->prog, "feather");
	shader->loc[static_cast<int>(GLNVGuniformLoc::INNERCOL)] = glGetUniformLocation(shader->prog, "innerCol");
	shader->loc[static_cast<int>(GLNVGuniformLoc::OUTERCOL)] = glGetUniformLocation(shader->prog, "outerCol");
	shader->loc[static_cast<int>(GLNVGuniformLoc::STROKEMULT)] = glGetUniformLocation(shader->prog, "strokeMult");
	shader->loc[static_cast<int>(GLNVGuniformLoc::TEX)] = glGetUniformLocation(shader->prog, "tex");
	shader->loc[static_cast<int>(GLNVGuniformLoc::TEXTYPE)] = glGetUniformLocation(shader->prog, "texType");
	shader->loc[static_cast<int>(GLNVGuniformLoc::TYPE)] = glGetUniformLocation(shader->prog, "type");
}
...
static int glnvg__setupPaint(struct GLNVGcontext* gl, struct NVGpaint* paint, struct NVGscissor* scissor, float width, float fringe)
{
	...
	if (paint->image != 0) {
		...
		glUniform1i(gl->shader.loc[static_cast<int>(GLNVGuniformLoc::TEXTYPE)], tex->type == NVG_TEXTURE_RGBA ? 0 : 1);
		...
		glUniform4fv(gl->shader.loc[static_cast<int>(GLNVGuniformLoc::INNERCOL)], 1, innerCol.rgba);
		glUniform4fv(gl->shader.loc[static_cast<int>(GLNVGuniformLoc::OUTERCOL)], 1, outerCol.rgba);
		glUniform1f(gl->shader.loc[static_cast<int>(GLNVGuniformLoc::STROKEMULT)], (width*0.5f + fringe*0.5f)/fringe);
							  ...
							  const float* bounds, const struct NVGpath* paths, int npaths)
{
	...
	if (npaths == 1 && paths[0].convex) {
		...
		glUseProgram(gl->shader.prog);
		glUniform1i(gl->shader.loc[static_cast<int>(GLNVGuniformLoc::TYPE)], static_cast<int>(GLNVGshaderType::SIMPLE));
		glUniform2f(gl->shader.loc[static_cast<int>(GLNVGuniformLoc::VIEWSIZE)], gl->viewWidth, gl->viewHeight);
								   ...
								   const struct NVGvertex* verts, int nverts)
{
	...
	glUseProgram(gl->shader.prog);
	glUniform1i(gl->shader.loc[static_cast<int>(GLNVGuniformLoc::TYPE)], static_cast<int>(GLNVGshaderType::IMG));
	glUniform2f(gl->shader.loc[static_cast<int>(GLNVGuniformLoc::VIEWSIZE)], gl->viewWidth, gl->viewHeight);
	glUniform1i(gl->shader.loc[static_cast<int>(GLNVGuniformLoc::TEX)], 0);
	glUniform1i(gl->shader.loc[static_cast<int>(GLNVGuniformLoc::TEXTYPE)], (tex != NULL && tex->type == NVG_TEXTURE_RGBA) ? 0 : 1);
